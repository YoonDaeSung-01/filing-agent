...
ACCOUNT_SYNONYMS: dict[str, set[str]] = {
    ...
    "영업이익": {"영업이익", "영업이익(손실)"},
    "법인세차감전순이익": {
        "법인세차감전 순이익",   # DART fnlttSinglAcnt 실제 표기(공백 포함)
        "법인세차감전순이익",    # 사용자 입력 표기(공백 없음)
        "법인세비용차감전순이익",
        "세전이익",
        "세전순이익",
    },
    "당기순이익": {"당기순이익", "당기순이익(손실)", "분기순이익", "반기순이익"},
    "총포괄손익": {"총포괄손익", "포괄손익", "포괄이익", "총포괄이익"},
    # ── 재무상태표(BS) ───────────────────────────────────────────────────────
    "자산총계": {"자산총계"},
    "부채총계": {"부채총계"},
    "자본총계": {"자본총계", "자기자본", "총자본", "순자산"},
    # ── 미지원: 현금흐름표(CF) 계정은 fnlttSinglAcnt 엔드포인트에 없음 ────
    # 영업활동현금흐름·투자활동현금흐름·재무활동현금흐름은 별도 API 필요.
}
...
def financial_lookup(
    ...
    지원 계정(손익계산서): 매출액, 영업이익, 법인세차감전순이익, 당기순이익, 총포괄손익.
    지원 계정(재무상태표): 자산총계, 부채총계, 자본총계.
    미지원: 영업활동현금흐름 등 현금흐름표 계정(별도 API 엔드포인트 필요).
